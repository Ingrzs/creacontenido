...
const UserManager = ({ users, activeUserId, onAddUser, onSelectUser, onDeleteUser }) => {
    ...
    const handleDeleteClick = () => {
         if (window.confirm('¿Eliminar usuario y todos sus perfiles? Esta acción no se puede deshacer.')) {
             onDeleteUser(activeUserId);
         }
    };
...
export const Controls = ({
    ...
    const canUseMixMode = activeUserId && profiles.length > 0;
    ...
    return (
        React.createElement('div', { className: "space-y-6" },
            ...
            React.createElement(ControlSection, { title: "Gestión de Cuenta" },
                React.createElement(UserManager, {
                    users: users,
            ...
            activeUserId ? (
                React.createElement(React.Fragment, null,
                    React.createElement(ControlSection, { title: "1. Selecciona Modo de Generación" },
                        React.createElement('div', { className: "flex flex-wrap gap-2" },
                            ...
                            React.createElement(ModeButton, { active: generationMode === 'ai-topic', onClick: () => setGenerationMode('ai-topic') }, "Por Tema"),
                            React.createElement(ModeButton, { active: generationMode === 'ai-trend', onClick: () => setGenerationMode('ai-trend') }, "Tendencia"),
                            React.createElement(ModeButton, { 
                                active: generationMode === 'ai-mix', 
                                onClick: () => canUseMixMode ? setGenerationMode('ai-mix') : alert('Necesitas guardar al menos un perfil en este usuario para usar el Modo Mix.'),
                                className: !canUseMixMode ? 'opacity-50 cursor-not-allowed' : 'border border-cyan-500'
                            }, "Mix de Marca (Multi-Perfil)")
                        )
                    ),
                    ...
                    generationMode.startsWith('ai') && (
                        React.createElement(ControlSection, { title: "2. Configura la Generación con IA" },
                            ...
                            // Si es modo MIX, mostramos solo el resumen
                            generationMode === 'ai-mix' ? (
                                React.createElement('div', { className: "space-y-4" },
                                    React.createElement('div', { className: "p-3 bg-cyan-900/30 border border-cyan-700 rounded-md" },
                                        React.createElement('h4', { className: "font-bold text-cyan-400 mb-2" }, "Modo Mix Activo"),
                                        React.createElement('p', { className: "text-sm text-gray-300 mb-3" }, 
                                            `Se generará 1 publicación por cada uno de tus ${profiles.length} perfiles guardados:`
                                        ),
                                        React.createElement('ul', { className: "list-disc pl-5 text-xs text-gray-400 space-y-1 max-h-40 overflow-y-auto" },
                                            profiles.map(p => React.createElement('li', { key: p.id }, `${p.name} (${p.niche})`))
                                        )
                                    ),
                                    // Aún permitimos configurar longitud y tema global opcional
                                     React.createElement('div', null,
                                        React.createElement(Label, { htmlFor: "ai-topic" }, "Tema Global (Opcional)"),
                                        React.createElement(Input, { id: "ai-topic", value: aiConfig.topic, onChange: (e) => handleAiConfigChange('topic', e.target.value), placeholder: "ej: Navidad, Ofertas de Verano"}),
                                        React.createElement('p', { className: "text-xs text-gray-400 mt-1" }, "Si se deja vacío, cada perfil usará sus propios microtemas.")
                                     ),
                                     React.createElement('div', null,
                                        React.createElement(Label, null, "Longitud de los textos"),
                                        React.createElement('div', { className: "flex gap-2" },
                                ...
                                // Configuración normal para Topic y Trend
                                React.createElement(React.Fragment, null,
                ...
                React.createElement('div', { className: "text-center p-6 bg-gray-800 rounded-lg" },
                    React.createElement('p', { className: "text-gray-400" }, "👈 Crea o selecciona una cuenta para comenzar.")
                )
            )
        )
    );
